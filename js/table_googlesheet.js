const ALERT_DEFAULT_VALUE = "Đã tải dữ liệu thành công\nĐang hiển thị dữ liệu...";
const ID_TABLE = "list";
    ...
    try {
        ...
        for (i = 1; i < arr.length; i++) {
            ...
            if (dateFilter) {
                ...
                if (new Date(date).getTime() === new Date(dateFilter).getTime()) {
                    debugger
                }    
            } else {
                tr = document.createElement('tr');
...
function showTable(documentId, resolve, reject) {
    briefcase.resolve = resolve;
    briefcase.reject = reject;
    briefcase.getCSV({
        id: documentId
    }, printElements);
